...
\begin{frame}{Applications existantes pour les tables interactives}
\setbeamercovered{invisible}
\begin{block}{{\small Pourquoi migrer les applications existantes?}}
\begin{itemize}
%	\item {\tiny Multitude de plateformes et d'applications}
	\item {\tiny Refactoring de applications existantes}
		\begin{itemize}
			\item {\tiny Réduire le temps et le coût de développement}
			\item {\tiny Faciliter le travail des développeurs en garantissant des applications utilisables}
		\end{itemize}
\end{itemize}
\end{block}
\pause
\begin{block}{{\small Pourquoi les tables interactives comme plateforme d'arrivée?}}
\begin{columns}[t]
...
\begin{frame}{Cas d'une application BD sur une table interactive }
 \begin{columns}
 	...
 	\pause
	\column{.6\textwidth}
{\small 	Une tabe interactive permet aux dessinateurs }
	\begin{itemize}
		\item {\tiny profiter d'un écran plus large et de nouveaux moyens d'interactions,} 
		\item {\tiny avoir un nouvel espace de conception qui facilite la collaboration}
	\end{itemize}
 \end{columns}
...
{\scriptsize 	Considérons que le NF de l'application à migrer est réutilisé} \\
	 {\tiny $\Rightarrow$ Hétérogénéité des plateformes~\cite{TanenbaumS.1978}}\\
\end{block}
\pause
\begin{block}{{\small  Nouvelle conception de l'UI en prenant compte les principes de conception}}
		\begin{itemize}
			\item {\scriptsize Comment réduire les coûts de mise en \oe{}uvre?}
		\end{itemize}
\end{block}
\pause
\begin{block}{{\small Déduire de l'UI à \textbf{partir du NF}{\tiny ~\cite{Kassoff2003}}}}
	\begin{itemize}
		\item {\scriptsize  Comment choisir et placer les composants graphiques et les moyens d'\textbf{interactions} en se basant sur le NF?\\}
		{\tiny $\Rightarrow$ Respect des critères ergonomiques!?}
	\end{itemize}
\end{block}
\pause
\begin{block}{{\small\textbf{Adapter} les éléments de l'UI de départ par rapport à la cible}}

\begin{itemize}
		\item {\scriptsize  Comment adapter la Structure, le Positionnement(Layout), le Style, les Interactions?}
...
\setbeamercovered{invisible}
		\begin{figure}[t]
		\begin{center}
		\includegraphics[ scale=.25]{img/tablesinteractives}
		\end{center}
		\end{figure}
...
\begin{columns}[t]
	\begin{column}{.4\textwidth}
		{\scriptsize { Utilisateurs }}
			\setbeamertemplate{itemize subitem}[circle]
			\begin{itemize}
				\item<2->  {\tiny Nombre d'utilisateurs}
				\item<2->  {\tiny Répartition des utilisateurs}
			\end{itemize}
		{\scriptsize { Instruments d'interactions }}
			\begin{itemize}
				\item<3->  {\tiny Tangibilité des interactions}
				\item<3->  {\tiny Tactibilité des interactions}
...
%				\item<4->  {\tiny Disposition de la surface d'affichage}
	...
	{\scriptsize { Corpus de guidelines~\cite{Microsoft2011}}}
		\begin{figure}[t]
		\begin{center}
		\includegraphics[ scale=.27]{../chap2/img-23-1}
		\end{center}
		\end{figure}
	\end{column}
...
\item {\scriptsize la prise en compte des guidelines pour assurer une UI respectant les\textbf{ critères ergonomiques,}}
...
{\scriptsize $\Rightarrow$  Mécanismes peu flexibles, réutilisables pour un ensemble  applications et produisant des UI peu utilisable  }\\
...
\begin{block}{{\scriptsize  CRF~\cite{Calvary2002}: Modèle pour la conception d'UI multi plateformes}}
\begin{itemize}
...
\end{flushleft}
   ...
   \begin{flushleft}{\scriptsize Basés sur une activité de Reverse Engineering(RE)} \newline
    $\Rightarrow $ {\tiny Modèles partiels de l'UI source sont retrouvés à partir de l'application à miger } 
   \end{flushleft} \\ 
...
%\begin{tabularx}{10cm}{|Y |Y| Y| }
%\hline {\scriptsize \textbf{Équivalences Statiques} des bibliothèques graphiques} 
% & {\scriptsize Prise en compte des  \textbf{guidelines} par des règles de transformation des modèles du CRF} 
% & {\scriptsize \textbf{Modèles } d'UI: Interactions, Structure, Layout} \\ 
%\hline 
%\end{tabularx} 
{\scriptsize $\Rightarrow$  Mécanismes réutilisables et génériques pour des plateformes et des applications mais produisant des UI peu conformes aux critères ergonomiques car moins flexible}
\end{frame}
...
\begin{itemize}
	...
	\item {\tiny Mécanismes d'aide à la migration d'UI plus flexible à moindre coût}
\end{itemize}
\end{block}
...
\end{itemize}
\end{block}
\begin{block}{{\small Mécanismes de migrations d'UI}}
	\begin{itemize}
...
\begin{frame}[t]{Primitives d'interactions}
\setbeamercovered{transparent}
\begin{block}{ {\small Interactions}}
\begin{figure}[t]
\begin{center}
\includegraphics[scale=.4]{../chap2/img-1}
\end{center}
\end{figure}
{\tiny $\Rightarrow$ Exprimer les actions, les commandes, les réponses et les feedback de l'UI par des interactions atomiques sur les \textbf{composants graphiques} : \textbf{Primitives d'interactions}}
\end{block}
{\tiny $ComposantGraphique=\langle Propri\acute{e}t\acute{e}s\ graphiques,\ Contenu(s),\ Lien\ avec\ NF \rangle $}
\pause

\begin{block}{{\small Primitives d'interactions}}
\begin{columns}[t]
\begin{column}{.4\textwidth}
	{\scriptsize Propriétés graphiques } %(Taille, Position)} 
	\begin{itemize}
	\item {\tiny Widget Move} \item {\tiny Widget Rotation } \item {\tiny Widget Resize} \item {\tiny Widget Selection, Navigation}
	\item {\tiny Widget Display}
	\end{itemize}
\end{column}
\pause
\begin{column}{.33\textwidth}
	{\scriptsize Contenu(s)} 
	\begin{itemize}
	\item {\tiny Data Edition} \item {\tiny Data Selection} \item {\tiny Data Move In} \item {\tiny Data Move Out } 
	\item {\tiny Data Display}
	\end{itemize}

\end{column}
\pause
\begin{column}{.33\textwidth}
{\scriptsize Lien avec le NF}
	\begin{itemize}
		\item {\tiny  Activation}
	\end{itemize}
\end{column}
...
% Modèles d'UI = Primitives d'interactions + Modèle de structure\\
	...
	\begin{figure}[h]
	\includegraphics[ scale=.37]{../chap5/modelsutilises-2}
...
\begin{columns}
	...
	\begin{itemize}
		\item{\scriptsize Les 6 opérateurs permettent d'établir des \textbf{équivalences dynamiques}}
		\item {\scriptsize Quels composants graphiques \textbf{sélectionner} si plusieurs correspondants?} 
		\item {\scriptsize Comment \textbf{transformer} l'UI source?}
	\end{itemize}
	\end{block}
	\end{column}
\end{columns}
...
\begin{block}{{\scriptsize Exemples Substitution n $ \rightarrow$ m}}
\begin{columns}
\begin{column}{.3\textwidth}
\only<2>{{\tiny  Substitution 4 à 4 en appliquant la guideline d'UI collaborative}}
\only<4> {{\tiny Substitution 2 à 1}}
\only<3> {{\tiny Substitution 1 à 2}}
\end{column}
\begin{column}{.7\textwidth} \only<2>{\includegraphics[scale=.4]{./img/transfo1to4}} 
\only<4>{\includegraphics[scale=.4]{./img/transfo2to1}} 
\only<3>{\includegraphics[scale=.3]{./img/transfo1to2}}
% &   \\ 
...
\item {\scriptsize Liens de contenance entre composants graphiques:}
\begin{itemize}
...
%\begin{enumerate}
{\tiny Supprimer les containers récursif imbriqués imbriqués et Appliquer la guideline d'utilisation en 360 degrés de l'UI (G7) sur tous les éléments }
%\end{enumerate}
\end{block}
\pause
...
%\begin{enumerate}
{\tiny Appliquer G7 Ou Si Container Homogène est un menu, alors appliquer les guidelines d'utilisation des Objets Tangibles (G2 et G3)}
%\end{enumerate}
\end{block}
\pause
\end{column}
...
%\begin{enumerate}
{\tiny Si parent est un Récursif alors cf. Règle 1 Sinon Aucune transformation}
%\end{enumerate}
\end{block}
\pause
...
%\begin{enumerate}
{\tiny Si tous les fils sont Container alors appliquer Règle 1.1\\}
...
\begin{figure}[t]
\begin{center}
\includegraphics[scale=.09]{./img/process}
...
\begin{block}{{\scriptsize Conclusion de la thèse}}

\end{block}
\pause
\begin{block}{{\scriptsize État d'avancement de la rédaction}}
\begin{tabular}{|l|l|} 	
\hline \textbf{Chapitres}  &  \textbf{État} \\ 
...
\hline {\tiny 3-Modélisation des interactions abstraites} 
					& {\tiny En cours (Lu par Audrey(1)}{\tiny Philippe(2)}  \\ 
...
\thispagestyle{empty}
\setcounter{page}{0}
\begin{block}{Critères de conformités aux guidelines}
\end{block}
\begin{block}{Charge de travail}
\end{block}
